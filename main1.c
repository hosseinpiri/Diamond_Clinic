...
void Open_and_Read_Files()
{
	...
	if ((err = fopen_s(&output_file, "C:/Users/Steven/Dropbox/Research Ideas/Diamond Clinic Reopening/Diamond Clinic/output.txt", "w")) != 0)
	{
		printf("The test output file was not opened.  Error #: %d \n", err);
		exit(0);
	}

	if ((err = fopen_s(&clinic_file, "C:/Users/Steven/Dropbox/Research Ideas/Diamond Clinic Reopening/Diamond Clinic/Clinic_Input_File.csv", "r")) != 0)
	{
		printf("The input file was not opened.  Error #: %d \n", err);
		exit(0);
	}

	if ((err = fopen_s(&elevator_file, "C:/Users/Steven/Dropbox/Research Ideas/Diamond Clinic Reopening/Diamond Clinic/Elevator_Input_File.csv", "r")) != 0)
	{
		printf("The input file was not opened.  Error #: %d \n", err);
		exit(0);
	}
...
void Load_Lobby_Arrivals()
{
	...
	for (i = 0; i < total_docs; i++)
	{
		...
		people[DOCTOR][i].arrive_to_elevator_time = max(people[DOCTOR][i].start_time - minutes_before_start_time, 0); //time 0 is the when the doors open, so can't arrive before then
		Load_Event(&event_head, people[DOCTOR][i].arrive_to_elevator_time, PERSON_ARRIVES_LOBBY, DOCTOR, i);
	}
	...
	for (i = 0; i < NUM_CLINICS; i++) //loop over each clinic
	{
		...
		{
			people[STAFF][total_staff].person_type = STAFF;
		...
		for (j = 0; j < clinics[i].num_docs; j++) //loop over each doctor working today in this clinic
		{
			...
			for (k = 0; k < clinics[i].num_pats[j]; k++)  //loop over each patient this doc will see today in this clinic
			{
				...
				people[PATIENT][total_pats].clinic = i;
